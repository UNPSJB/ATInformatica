var contador_proceso = 0;
var aux;
var tiles_divs = [];

function _imprimirPDF(contenido_rep){
    var contenido_default = {
        membrete: 'AT Informática\nFragata Hércules 596\nPuerto Madryn (9120) - Chubut\nTel: (0280) 4456800',
        titulo: '', // Título del reporte
        tiles: [    // Lista de tiles
            {
                tipo: 'vacio'
            },
        ],
        usuario: '[usuario]',   // Usuario que solicitó el reporte
        fechahora: '01/01/2007 00:00'   // Fecha y hora (placeholder, se pone automáticamente en now())
    };

    // Mergear opciones
    var contenido = $.extend(contenido_default, contenido_rep);

    // Init doc
    var doc = new jsPDF({
        orientation: 'p',
        unit: 'mm'
    });

    var num_pag = 1;
    
    // Funciones auxiliares para alinear texto
    var centrar_punto = function(text, x, y) {
        var textWidth = doc.getStringUnitWidth(text) * doc.internal.getFontSize() / doc.internal.scaleFactor;
        var textOffset = x - (textWidth / 2);   // Centrar 
        doc.text(textOffset, y, text);
    }
    var centrar_ancho = function(text, y) {
        var textWidth = doc.getStringUnitWidth(text) * doc.internal.getFontSize() / doc.internal.scaleFactor;
        // Me caga esta cuenta matemática:
        var textOffset = (doc.internal.pageSize.width / 2) - (textWidth / 3);   // Centrar según el ancho de la página
        doc.text(textOffset, y, text);
    }
    var derecha_punto = function(text, x, y) {
        var textWidth = doc.getStringUnitWidth(text) * doc.internal.getFontSize() / doc.internal.scaleFactor;
        var textOffset = x - textWidth;   // Alinear a la derecha
        doc.text(textOffset, y, text);
    }

    var cabecera = function() {
        // Agregar logo (hardcode por ahora)
        doc.addImage('data:image/jpeg;base64,', 'JPG', 0, 2, 25, 20);
        
        // Texto para el "membrete"
        doc.setFontSize(10);
        doc.text(contenido['membrete'], 35, 8);

        // Línea horizontal del encabezado
        doc.line(10, 27, 200, 27);
    }

    var pie_pag = function(numero) {
        // Línea horizontal del pie de página
        doc.line(10, 288, 200, 288);
        
        // Pie de página
        var currentdate = new Date(); 
        var datetime = currentdate.getDate() + "/"
                    + (currentdate.getMonth()+1)  + "/" 
                    + currentdate.getFullYear() + " @ "  
                    + currentdate.getHours() + ":"  
                    + currentdate.getMinutes() + ":" 
                    + currentdate.getSeconds();

        doc.setFontSize(9);
        doc.text('Pag. ' + numero, 15, 293);
        centrar_ancho(contenido['usuario'], 293);
        derecha_punto(datetime, 205, 293);
    }

    cabecera();
    pie_pag(num_pag);    


     /**
      * TÍTULO GENERAL - PRIMERA PÁGINA
      * TODO: multilínea ¯\_(ツ)_/¯
      */
    var titulo_general = contenido['titulo'];
    doc.setFontSize(20);
    centrar_ancho(titulo_general, 30);  // Centrar en la página

    doc.setFontSize(14);
    
    // Posición inicial del cursor
    var margen_izquierdo = 30;
    var cursor_graficos = {x: margen_izquierdo, y: 60};

    // Aux
    var tamaño_x = 0, tamaño_y = 0, espacio_entre_tiles, tamaño_ultimo_x = 0, tamaño_ultimo_y = 0;
        
    function check_pagina() {
        // Avanzar de página si mi cursor en y no me permite dibujar a lo alto
        // Verificación: ¿cursor + tamaño que necesito > area de dibujo?
        // Si es así, no tengo lugar, y debo avanzar de página.
        if (cursor_graficos.y + tamaño_y > 290) {
            console.log('Avance pagina - check: ', cursor_graficos.y, ' + size.y = ', tamaño_y);
            // Agregar página
            doc.addPage();
            
            // Reset cursores
            cursor_graficos.x = margen_izquierdo;
            cursor_graficos.y = 30;

            // Redibujar cabecera y pie
            cabecera();
            pie_pag(++num_pag);
        }
    }

    function check_fila() {
        // Avanzar de fila si mi cursor en x no me permite dibujar a lo ancho
        // Verificación: ¿cursor + tamaño que necesito > area de dibujo?
        // Si es así, no tengo lugar, y debo avanzar de fila.
        if (cursor_graficos.x + tamaño_x > (doc.internal.pageSize.width - espacio_entre_tiles)) {
            console.log('Avance fila. CHECK CURSOR: ', cursor_graficos, ' - CHECK LAST_SIZE: ', tamaño_ultimo_x, tamaño_ultimo_y);
            // Reiniciar posición x
            cursor_graficos.x = margen_izquierdo;
            // Setear posición y
            cursor_graficos.y = cursor_graficos.y + tamaño_ultimo_y + espacio_entre_tiles;
            // Resetear último tamaño_y
            tamaño_ultimo_x = 0;
            tamaño_ultimo_y = 0;
        }
    }

    // ~~ Procesar los tiles ~~
    contenido.tiles.forEach(function(tile) {
        // Tamaño de un tile por defecto
        tamaño_x = 85;
        tamaño_y = 100;

        // Margen
        espacio_entre_tiles = 4; // Fijo.

        // Es ancho? Setear tamaño_x y redefinir el desplazamiento (tile 1x2, sólo nos moveremos hacia abajo)
        if (tile.ancho)
            tamaño_x = tamaño_x * 2;

        // Linea divisoria? Dibujar y break
        if (tile.tipo == 'hr') {
            tile.ancho = true;
            check_fila();
            doc.line(margen_izquierdo, cursor_graficos.y + 2, doc.internal.pageSize.width - espacio_entre_tiles, cursor_graficos.y + 2);
            console.log('Drawn line at y=', cursor_graficos.y + 2);
            cursor_graficos.y += espacio_entre_tiles * 2;
            return;
        }

        check_fila();        
        
        // Si soy un gráfico, pasarme a base64 y agregarme bajo el cursor
        if (tile.tipo == 'grafico') {
            var canvas;
            var canvasImg = '';

            if (tile.canvas) {
                canvas = tile.canvas;
                canvasImg = canvas.toDataURL();
            } else if (tile.selector) {
                canvas = ($(tile.selector).find('canvas').first()[0]);
                canvasImg = canvas.toDataURL();
            }

            // Sección de cuentas matemáticas para escalar proporcionalmente los gráficos
            var alto = canvas.height;
            var ancho = canvas.width;

            // Relación de aspecto del gráfico (proporción ancho/alto)
            var rel_grafico_ancho = ancho / alto;
            // Relación de aspecto del gráfico (proporción alto/ancho)
            var rel_grafico_alto = alto / ancho;

            // Relación de aspecto del tile
            var rel_tile = tile.ancho ? 2 : 1;

            // Si el gráfico es menos ancho que mi tile, fijar la altura y ver en cuánto cambio su ancho
            // para la inserción según el tamaño del tile
            if (rel_grafico_ancho <= rel_tile) {
                // Determinar ancho nuevo basado en la altura fijada (tamaño_y)
                tamaño_x = tamaño_y * rel_grafico_ancho;
            } else {    // Si el gráfico es más ancho que mi tile, fijo el ancho al máximo y modifico la altura
                // Determinar alto nuevo basado en el ancho fijado (tamaño_x)
                tamaño_y = tamaño_x * rel_grafico_alto;
            }

            // Ahora que determiné el tamaño, ¿cabe en la página?
            check_pagina();

            // Poner la maldita cosa en el reporte:
            doc.addImage(canvasImg, 'PNG', cursor_graficos.x, cursor_graficos.y, tamaño_x, tamaño_y);
            console.log('Grafico insert at ' + cursor_graficos.x + ', ' + cursor_graficos.y + ' | Sized: ' + tamaño_x + ' x ' + tamaño_y);

        } else if (tile.tipo == 'cod_html') {   // Si tengo HTML, escupirlo en la hoja
            var fuenteHTML = tile.html;

            doc.fromHTML(
                fuenteHTML,
                cursor_graficos.x,
                cursor_graficos.y,
                {
                'width': tamaño_x, 
                'elementHandlers': {
                    '.no_imprimir': function(element, renderer) {
                        return true;
                        }
                    }
                }
            );
            console.log('HTML insert at ' + cursor_graficos.x + ', ' + cursor_graficos.y);
        }

        // Actualizar valor del cursor de gráficos en sentido de lectura 
        cursor_graficos.x = cursor_graficos.x + tamaño_x + espacio_entre_tiles;

        tamaño_ultimo_x = tamaño_x;
        tamaño_ultimo_y = Math.max(tamaño_ultimo_y, tamaño_y);
    });

    doc.save('prueba.pdf');
}

/**
 * La macumba más oscura - async -> sync
 * TODO: REHACER COMO CORRESPONDE MÁS ADELANTE.
 * 
 * Interfaz:
 * imprimirPDF({
 *  membrete: [string] // \n hace newline
 *  titulo: [string],
 *  tiles: [tile[]]
 *  usuario: [string]
 * })
 * 
 * tile = {
 *  tipo: [string] = 'vacio' | 'grafico' | 'cod_html' | 'div' | 'hr'
 *  titulo: [string]
 *  selector: [string]
 *  html: [string]
 *  ancho: [boolean]
 * }
 */
function imprimirPDF (contenido, index) {
    var ind = 0;
    if (index != undefined) {
        if (index == -1) {
            _imprimirPDF(contenido);
            return
        }
        ind = index + 1;
    } else {    // No pasé index, primera ejecución
        // Guardar ref global
        aux = contenido;

        // Contar todas los divs de una
        contenido.tiles.forEach(function(e) {
            if (e.tipo == 'div')
                contador_proceso++;
        });
    }

    for (var i = ind; i < contenido.tiles.length; i++) {
        if (contenido.tiles[i].tipo == 'div') {
            var div = ($(contenido.tiles[i].selector)[0]);
            tiles_divs.push(i);
            html2canvas(div).then(function(canvas) {
                contenido.tiles[tiles_divs[0]].tipo = 'grafico';
                contenido.tiles[tiles_divs[0]].canvas = canvas;
                tiles_divs.splice(0,1);
                contador_proceso--;

                if (contador_proceso == 0) {
                    imprimirPDF(aux, -1);
                }
            });

            imprimirPDF(contenido, i);
            break;
        }
    };
}